...
import com.aengine.util.StringUtils;
...
public class CronSequenceGenerator {
	...
	public Date next(Date date) {
		...
		2 If seconds model move on, otherwise find the next model:
		2.1 If next model is in the next minute then roll forwards

		3 If minute matches move on, otherwise find the next model
		3.1 If next model is in the next hour then roll forwards
		3.2 Reset the seconds and go to 2

		4 If hour matches move on, otherwise find the next model
		4.1 If next model is in the next day then roll forwards,
